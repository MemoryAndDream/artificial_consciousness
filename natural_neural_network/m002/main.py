...
def test_see(baby_mind: Mind):
    # 测试事件 通过对神经冲动的追踪反映思维
    # 事件： 妈妈出现了
    eye_node = baby_mind.get_node('eye1')
    eye_node.active_level = 100

    # eye_node.act()
    active_nodes = [eye_node]

    for time in range(10):
        print(f'time {time}')
        for node in active_nodes:
            for link in baby_mind.all_links:  # 暂时不考虑多link同时作用，其实应该加起来
                if link.source_id == node.id:
                    active_node = baby_mind.get_node(link.target_id)
                    active_node.active_level += link.strength

        active_nodes = []
        for node in baby_mind.all_nodes:
            if node.active_level >= 50:  # 设置激活阈值
                active_nodes.append(node)
                node.active_level = 0
                print(f'active {node.id}')

def test_hear(baby_mind:Mind):
    # 测试回忆 其实也是事件
    # 事件，听到妈妈这个词
    print('event start')
    eye_node = baby_mind.get_node('ear1')
    eye_node.active_level = 100
...
def main():
    ...
    # test_hear(baby_mind)
    # 测试结果：训练一遍时，只有阈值为2才能激活mouth3 ear2 ear3
    # 训练10遍时候甚至可以设置阈值50，就这样还会无限循环 必须引入疲劳机制

    test_see(baby_mind)

    # 但是感觉如果不是逻辑错误的话，这个例子已经差不多了
