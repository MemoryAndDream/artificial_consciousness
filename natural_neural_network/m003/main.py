...
# 期望结果：按顺序输出apple
class Node:
    def __init__(self,name): # id 必须为唯一
        self.id=id(self)#
        self.name=name
        # self.id = name
        self.active_level = 0
...
class Mind:
    ...
    def init_sub_nodes(self, node: Node):
        # generate level 2,3 node for the first level node
    ...
    def activate_node(self, init_node_id):
        # init node被视为接收输入的node,所以处理事件只需要激活这一类node即可
        init_node = self.all_nodes[init_node_id]
